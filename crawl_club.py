...
for league, url in zip(league_name, league_url):
    for season in range(2015, 2022): # 2015, 2022 | dữ liệu đến năm max = 2021
        print(f"Start: league {league} - season {season}")
        t1 = time.time()
        ...
        top_team = [team.get('href').split('/')[-3] for team in soup.select("#yw3 .hauptlink a:nth-child(1)")] 
        ...
        avrage_market_value = [market_value.text for market_value in soup.select("tbody .zentriert+ .rechts")] #ok
        club.loc[(club['Season'] == season) & (club['League'] == league), 'avgMarketValue'] = avrage_market_value

        # add totalMarketValue to club dataFrame 
        total_market_value = [total.text for total in soup.select("tbody .rechts+ .rechts")] #ok
        ...
        for club_url in club_urls:
            ...
            players_list = soup2.select(".inline-table .hauptlink > a") # dang bi sai
            ...
            ### sua doan nay
            players.loc[(players['ClubID'] == club_id) & (players['Season'] == season), 'Player_name'] = ';'.join(p.text[2:].strip() for p in players_list)
            ###
            ...
            player_main_position = [position.text[2:].strip() for position in soup2.select(".inline-table tr+ tr td")]
            players.loc[(players['ClubID'] == club_id) & (players['Season'] == season), 'Player_position'] = player_main_position
            ...
            champoin_list = [ch.text for ch in soup3.select("td:nth-child(1) , .no-border-links")] #cup vo dich
            victories = []
        ...
        t2 = time.time()
        print(f"Time: {t2 - t1}")
        print(f"Done: league {league} - season {season}")

club = pd.merge(club, club_table_league, on=["League","Season","ClubID"])

club.to_csv('/kaggle/working/club.csv')
players.to_csv('/kaggle/working/club_players.csv')
league_goals.to_csv('/kaggle/working/league_goals.csv')
pd.DataFrame(All_Players_Link).drop_duplicates().to_csv("/kaggle/working/players_link.csv", index = False)
